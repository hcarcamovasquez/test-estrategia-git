...
#Region "Private Variables"
        Public Shared ADAdminUser As String = "Administrator"
        Public Shared ADAdminPassword As String = "mote.123"
        'This needs to have the domain name or IP address on this line
        Public Shared ADFullPath As String = "LDAP://192.168.50.147"

        'This must be the domain name of the domain controller (not the computer name, just the domain name)
        Public Shared ADServer As String = "MONEDA.LOCAL"
...
#End Region
        ...
        Public Shared Function GetDirectoryEntry() As DirectoryEntry
            ...
            dirEntry.Path = "LDAP://192.168.50.147/CN=Users;DC=MONEDA.LOCAL"
            dirEntry.Username = "moneda\administrator"
            dirEntry.Password = "mote.123"
            Return dirEntry
        End Function
        ...
        Public Shared Function Login(ByVal UserName As String, ByVal Password As String) As ADWrapper.LoginResult
            ...
            ADFullPath = "LDAP://" + DirectCast(configurationAppSettings.GetValue("ADDomainIP", GetType(System.String)), String)
            ...
            If IsUserValid(UserName, Password) Then
                Dim dirEntry As DirectoryEntry = GetUser(UserName, Password) ' GetUser(UserName)
...
#Region " GetUser Methods "
        ...
        Public Shared Function GetUser(ByVal UserName As String, ByVal Password As String) As DirectoryEntry
            ...
            If Not searchResults Is Nothing Then
                ...
                dirEntry = New DirectoryEntry(searchResults.Path, UserName, Password, AuthenticationTypes.Secure)
                Return dirEntry
            Else
                Return Nothing
            End If
...
#Region " Helpers "
        ...
        End Function
#End Region
        Shared Function ListUsers2(domain As String, ou As String, user As String, pass As String) As List(Of String)
            Dim lista As List(Of String) = New List(Of String)


            If IsUserValid(user, pass) Then
                ...
                For Each result In srch.FindAll()
                    ...
                    lista.Add(dir.Properties("cn").Value)
                Next
            End If
        ...
        Shared Function ListUSers(domain As String, ou As String, user As String, pass As String) As List(Of String)
            Dim lista As List(Of String) = New List(Of String)
            If IsUserValid(user, pass) Then
                Dim EntradasEnElDirectorio As New DirectoryEntry("LDAP://" & domain)
                EntradasEnElDirectorio.Password = pass
                EntradasEnElDirectorio.Username = user
                'Nuestro buscador
                Dim BusquedaEnElDirectorio As New DirectorySearcher(EntradasEnElDirectorio)
                'Filtro de busqueda


                BusquedaEnElDirectorio.Filter = “(&(ObjectClass=user)(objectCategory=person))”
                Dim result As SearchResult
                Dim sname As String
                Dim sgivenname As String
                Dim samaacoutnname As String
                Dim cn As String

                Dim direEntry As System.DirectoryServices.DirectoryEntry

                For Each result In BusquedaEnElDirectorio.FindAll()
                    direEntry = result.GetDirectoryEntry

                    cn = direEntry.Properties("cn").Value
                    sname = direEntry.Properties("SN").Value
                    sgivenname = direEntry.Properties("givenName").Value
                    samaacoutnname = direEntry.Properties("SAMAccountName").Value

                    'lista.Add("[" + cn + "] [" + sname + "] [" + sgivenname + "] [" + samaacoutnname + "]")
                    lista.Add(samaacoutnname)

                Next
                ' Dim Entradas = From usuario As SearchResult In BusquedaEnElDirectorio.FindAll() Select usuario.GetDirectoryEntry

            End If
            Return lista
        End Function
        Public Shared Function GetAllUsers(ByVal ldapServerName As String) As Hashtable

            'To retrieve list of all  LDAP users 

            'This function returns HashTable
            ' _ldapServerName = ldapServerName
            ldapServerName = "192.168.50.147"
            ' ListUsers(ldapServerName, "cn=Users,dc=moneda,dc=local")
            Dim sServerName As String = "MONEDA"

            Dim oRoot As DirectoryEntry = New DirectoryEntry("LDAP://" & ldapServerName &
       "/cn=Users,dc=moneda,dc=local")
            oRoot.Password = "mote.123"
            oRoot.Username = "administrator"
            ...
            Try
                oSearcher.Filter = "(&(objectClass=user)(objectCategory=person))"
                ...
                For Each oResult In oResults
                    de = oResult.GetDirectoryEntry()
                    If Not de.Properties("cn").Value = "" Then

                        RetArray.Add(de.Properties("users").Value, de.Properties("cn").Value)
                    End If
                Next
